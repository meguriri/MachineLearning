...
def main():
    dataSetName = r"./adult/adult_mini.data"
    ...
    print(Tree)
    storeTree(Tree, r'./model/true.txt')  # 保存决策树，避免下次再生成决策树

    # Tree=grabTree(r'./model/true.txt')#读取决策树，如果已经存在tree.txt可以直接使用决策树不需要再次生成决策树
    total, correct, error  = test(Tree, label, r'./adult/adult.test',labelType,mydate)
